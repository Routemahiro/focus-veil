...
- 画面の動きに合わせて周りが明るくなる Motion highlight は削除済みです。メインスポットはマウス周辺に残します。
...
- 2026-06-09時点で、トレイメニュー、軽量な設定保存、IPC sender検証、CSP、navigation/window.open制限を追加しています。
...
| 注目補助 | マウススポット | Webカメラ視線推定 / 画面動き追従の明るさ | 視線推定は精度・権限・負荷のコストが大きく、動き追従ハイライトは視線が散るため |
...
設定はメインプロセスを単一ソースにし、ElectronのuserData配下の `settings.json` へ保存します。対象は暗幕の透明度、スポットサイズ、スポット境界の柔らかさ、Focus/Break分数、Ripple effects、Overlay enabled、Start with Windows、Auto-updateです。smoke実行時は保存を無効化し、検証結果がユーザー設定に影響しないようにしています。`Start with Windows` は既定 OFF で、インストール済み Setup だけが `app.setLoginItemSettings({ openAtLogin })` を呼びます。ポータブル / 開発起動では登録せず、操作メニューにその旨を出します。自動更新は Windows の Setup インストール版だけが GitHub Releases を確認します。署名検証は行いません。`Auto-update` は自動確認の ON/OFF で、その直下の `Check for updates` は今すぐの確認です。ポータブル / 開発起動では electron-updater を呼ばず、コントロールに入れられない旨を出したうえで GitHub Releases を開くか聞きます。ブラウザを開くのは Yes のときだけです。ダウンロード中だけ右下タイマーの下に細い進捗バーを出し、完了や未ダウンロードでは消します。入れたあとは従来の `Restart to Update` と Quit です。
...
- 描画はactive/idleの2段階です。マウス移動直後、スポット未収束、通知演出中、操作モード中は約30fpsで描画し、静止アイドル時は約10fpsへ落とします。
- 水面は低密度の線描画だけで、WebGL、粒子、大量DOMを使っていません。
- タイマーIPCは、タイマー実行中だけ250ms間隔で送ります。停止中は操作や設定変更などの状態変化時だけ送ります。
- Overlay disabled時はCanvasを1回clearし、描画ループを停止します。タイマーUIと復帰導線は維持します。
