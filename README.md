...
初期テーマは控えめな水面です。低速・低コントラスト・低密度のCanvas描画に留め、ポモドーロの残り時間と静かな通知演出で集中/休憩の切り替えを支援します。実験機能として、画面内の動きがある領域を低解像度で検出し、マウス位置より自然に「注目していそうな場所」へスポットライトを寄せます。
...
- `npm run smoke`: 成功。Start/Pause/Reset、短時間タイマー遷移、motion focusの疑似ターゲット、通知演出、4枚のスクリーンショット生成を確認。
...
- Motion focusは画面キャプチャが使えない環境や保護された画面では自動的にマウス追従へ戻ります。動画、広告、ローディングなどの動きに引っ張られる可能性があります。
...
- Motion focusのしきい値を、動画、スクロール、タイピング、コード編集などの実作業で調整する。
